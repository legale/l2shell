# l2shell
L2-оболочка: клиент и сервер обмениваются ethernet-кадрами с пользовательским EtherType (`0x88B5`) и передают HELLO/команды в обход IP-стека.
...
`make test-unit` гоняет юнит-тесты, `make test` (с `sudo`) — e2e-скрипт на veth, `make kmod` собирает модуль ядра `l2shell_kmod.ko`.
