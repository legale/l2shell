...
static void server_send_tagged(server_ctx_t *ctx, const char *tag, const char *fmt, ...) {
    ...
    frame_len = l2s_send_frame_to_socket(ctx->sockfd, &ctx->peer_addr, &meta, packet.payload, (size_t)payload_len, "server_tx status frame ");
    if (frame_len < 0) {
...
static void server_send_ready_ack(server_ctx_t *ctx, const hello_view_t *hello) {
    ...
    meta.type = L2S_MSG_CONTROL;
    meta.flags = 0;
    frame_len = l2s_send_frame_to_socket(ctx->sockfd, &ctx->peer_addr, &meta, packet.payload, msg_len, "server_tx ready ack frame ");
    if (frame_len < 0) {
...
static ssize_t server_socket_event_handler(server_ctx_t *ctx, pack_t *packet) {
    ...
    parse_rc = l2s_parse_frame(packet, (size_t)ret, CLIENT_SIGNATURE, &payload_size);
...
static void server_handle_shell_event(server_ctx_t *ctx, pack_t *packet) {
    ...
    int frame_len = l2s_send_frame_to_socket(ctx->sockfd, &ctx->peer_addr, &meta, packet->payload, (size_t)ret, "server_tx frame ");
    if (frame_len < 0) {
