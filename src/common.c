...
static void log_internal(const char *level, const char *tag, const char *fmt,
                         va_list ap) {
  if (!level)
    level = "info";
  if (!tag)
...
int l2s_send_frame_to_socket(int sockfd, const struct sockaddr_ll *dst,
  ...
  int frame_len =
      l2s_build_frame(&packet, sizeof(packet), meta, payload, payload_len);
  if (frame_len < 0)
    return frame_len;
