...
static u64 client_calc_heartbeat_interval_ns(int idle_timeout_sec) {
  int secs = idle_timeout_sec;
  if (secs <= 0)
...
static int parse_ready_message(const u8 *payload, size_t len,
                               ready_msg_t *msg) {
  char buf[128];
  size_t copy;
...
static inline int a2mac(const char *s, u8 mac[ETH_ALEN]) {
  ...
  if (!s)
    return -1;
  int n = sscanf(s, "%02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx", &v[0], &v[1],
                 &v[2], &v[3], &v[4], &v[5]);
  if (n != ETH_ALEN)
    return -1;
...
static int init_client_sock(client_ctx_t *ctx, const char *iface) {
  ...
  if (init_packet_socket(&ctx->sockfd, &ctx->ifr, &ctx->bind_addr, iface, 0) !=
      0) {
    return -1;
  }
...
static int client_ctx_init(client_ctx_t *ctx, const client_args_t *args) {
  ...
  ctx->heartbeat_interval_ns =
      client_calc_heartbeat_interval_ns(args->idle_timeout);
...
static int client_recv_packet(client_ctx_t *ctx, l2s_frame_t *pack,
                              int *payload_len) {
  ...
  got = recvfrom(ctx->sockfd, pack, sizeof(*pack), 0, (struct sockaddr *)&peer,
                 &plen);
  if (got < 0) {
...
/* tx frame */
static int client_send_payload(client_ctx_t *ctx, const void *payload,
                               size_t payload_len) {
  ...
  int frame_len =
      l2s_send_frame_to_socket(ctx->sockfd, &ctx->saddr, &meta, payload,
                               payload_len, "client_tx frame ");
  if (frame_len < 0) {
...
static int client_send_heartbeat(client_ctx_t *ctx) {
  u8 payload[MAX_DATA_SIZE] = {0};
  int len = hello_build_heartbeat(payload, sizeof(payload));
  if (len < 0) {
...
static int client_send_hello(client_ctx_t *ctx, const client_args_t *args,
                             u64 *nonce_out) {
...
static int client_wait_socket_ready(client_ctx_t *ctx, u64 deadline_ns,
                                    const char *log_tag) {
  assert(ctx);
  while (1) {
    u64 now = l2s_mono_ns();
  ...
  while (1) {
    ...
    if (parse_ready_message(pack.payload, (size_t)payload_len, &ready_msg)) {
      ...
      log_info("client_wait_ready",
               "event=ready_ignored expected=%016llx recv=%016llx has_nonce=%d "
               "src=userland:%d kernel:%d",
               (unsigned long long)expected_nonce,
               (unsigned long long)ready_msg.nonce, ready_msg.have_nonce,
               ready_msg.from_userland, ready_msg.from_kernel);
      continue;
    }
  }
}
...
static int client_handshake(client_ctx_t *ctx, const client_args_t *args) {
  ...
  while (attempt < max_attempts) {
    ...
    if (ready == 2) {
      log_info("client_handshake", "event=kernel_ack nonce=%016llx",
               (unsigned long long)nonce);
      attempt++;
      continue;
    }
...
static int client_handle_stdin_event(client_ctx_t *ctx) {
  ...
  if (client_send_payload(ctx, ibuf, (size_t)r) != 0)
    return -1;
  return 1;
}
...
static int client_loop(client_ctx_t *ctx) {
  ...
  for (;;) {
    ...
    struct timeval tv = {
        .tv_sec = (time_t)(wait_ns / NSEC_PER_SEC),
        .tv_usec = (suseconds_t)((wait_ns % NSEC_PER_SEC) / NSEC_PER_USEC),
    };
    ...
    int stdin_activity = 0;
    if (FD_ISSET(STDIN_FILENO, &rfds)) {
      int handled = client_handle_stdin_event(ctx);
      if (handled < 0)
        return -1;
      if (handled > 0)
        stdin_activity = 1;
...
int client_main(int argc, char **argv) {
  ...
  if (a.cmd) {
    ...
    if (seen <= 0) {
      if (seen == 0)
        log_error("client_wait", "event=no_response timeout_ns=%llu",
                  (unsigned long long)RESPONSE_TIMEOUT_NS);
      client_ctx_deinit(&ctx);
      if (interactive)
        client_restore_stdin();
      return 1;
