...
static int l2s_validate_build_args(const l2s_frame_t *frame,
...
int l2s_build_frame(l2s_frame_t *frame, size_t frame_capacity,
  ...
  size_t frame_len;
  int rc = l2s_validate_build_args(frame, frame_capacity, meta, payload,
                                   payload_len);
  if (rc != L2S_FRAME_OK)
    return rc;
...
int l2s_parse_frame(l2s_frame_t *frame, size_t frame_len,
  ...
  if (frame_len < expected_len) {
    log_error("packet", "event=frame_truncated len=%zu need=%zu", frame_len,
              expected_len);
    return L2S_FRAME_ERR_SHORT;
  }
  ...
  if (crc_host != crc_calc) {
    log_error("packet", "event=crc_mismatch recv=%u calc=%u", crc_host,
              crc_calc);
    return L2S_FRAME_ERR_CRC;
  }

  if (data_len > 0) {
    u8 nonce[PACKET_NONCE_LEN];
    memcpy(nonce, nonce_ptr, PACKET_NONCE_LEN);

    enc_dec(data_ptr, data_ptr, l2s_shared_key, data_len);
    for (size_t i = 0; i < data_len; i++)
      frame->payload[i] = data_ptr[i] ^ nonce[i & (PACKET_NONCE_LEN - 1)];
  }
...
int build_packet(l2s_frame_t *packet, size_t payload_size,
                 const u8 src_mac[ETH_ALEN], const u8 dst_mac[ETH_ALEN],
                 u32 signature) {
...
int parse_packet(l2s_frame_t *packet, ssize_t frame_len,
