...
static size_t store_spawn_cmd(const u8 *payload, size_t len,
                              const hello_view_t *hello) {
  size_t stored = 0;

  if (hello && hello->server_started && hello->server_bin_path &&
      hello->server_bin_path_len > 0) {
    stored =
        store_cmd_bytes(hello->server_bin_path, hello->server_bin_path_len);
...
retry_open:
  ...
  ret = l2sh_lz4_decompress(l2shell_embed, l2shell_embed_len, decomp,
  ...
  while (remaining > 0) {
    ssize_t wrote = kernel_write(
        filp, decomp + (l2shell_embed_orig_len - remaining), remaining, &pos);
    if (wrote < 0) {
      ret = (int)wrote;
      goto out_free;
    }
...
out_close:
  ...
  pr_info("l2sh: refreshed embedded server at %s size=%u (compressed=%u)\n",
          L2SHELL_TMP_PATH, l2shell_embed_orig_len, l2shell_embed_len);
  return 0;
}
...
static int build_server_packet(struct pack *pkt, const u8 *src_mac,
                               const u8 *dst_mac, const u8 *payload,
                               size_t payload_len) {
...
static void send_ready_ack(struct net_device *dev, const u8 dst_mac[ETH_ALEN],
                           const struct hello_view *hello) {
  struct pack *pkt;
  struct sk_buff *skb;
  char payload[64];
  size_t payload_len;
  ...
  if (hello && hello->have_nonce) {
    ...
    payload_len = scnprintf(payload, sizeof(payload), "ready source=kernel\n");
  }
...
static void exec_server(struct work_struct *work) {
  static char *envp[] = {"HOME=/tmp", "TERM=xterm-256color", NULL};
...
static bool kmod_should_drop_frame(struct net_device *dev, size_t frame_len,
                                   u32 checksum) {
  ...
  if (!frame_dedup_should_drop(&g.dedup, frame_len, checksum, cur_ifindex, now,
                               KMOD_DEDUP_WINDOW_NS, &prev_ifindex, &age_ns)) {
    return false;
  }

  l2sh_info(
      "dedup drop len=%zu checksum=%u iface=%s prev_ifindex=%d age_ns=%llu\n",
      frame_len, checksum, dev ? dev->name : "<unknown>", prev_ifindex, age_ns);
  return true;
}
...
static int l2_rx(struct sk_buff *skb, struct net_device *dev,
...

  {
    ...
    if (plain_len > 0) {
      preview_len = plain_len > sizeof(cipher_preview) ? sizeof(cipher_preview)
                                                       : plain_len;
    ...
    if (crc_calc != crc_recv) {
      ...
      print_hex_dump(KERN_INFO, "l2sh: crc nonce ", DUMP_PREFIX_NONE, 16, 1,
                     nonce_buf, PACKET_NONCE_LEN, false);
      if (preview_len > 0) {
        print_hex_dump(KERN_INFO, "l2sh: crc payload enc ", DUMP_PREFIX_NONE,
                       16, 1, cipher_preview, preview_len, false);
        print_hex_dump(KERN_INFO, "l2sh: crc payload dec ", DUMP_PREFIX_NONE,
                       16, 1, data_ptr, preview_len, false);
      }
      return NET_RX_SUCCESS;
    }
    ...
    if (plain_len > 0) {
      ...
      if (!g.launch_pending) {
        size_t used =
            store_spawn_cmd(nonce_ptr, plain_len, hello_ok ? &hello : NULL);
        if (!used) {
          g.cmd_buf[0] = '\0';
...
static void l2sh_force_promisc(const char *ifname) {
  ...
  rc = dev_change_flags(dev, dev->flags | IFF_UP | IFF_PROMISC | IFF_ALLMULTI,
                        NULL);
  rtnl_unlock();
  if (rc == 0)
...
#define WRITE_KOBJ(name, value)                                                \
  do {                                                                         \
    int ret = snprintf(path, sizeof(path), "/sys/class/net/%s/queues/%s",      \
                       ifname, name);                                          \
