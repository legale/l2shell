...
- `client.c` — реализация L2-клиента: принимает имя удаленной команды третьим позиционным аргументом (`/bin/bash`, `/bin/sh` и т.д.) и шлет init-кадр с этим значением (можно отключить через `--skip-init`, если удаленная сторона уже запущена). Читает ввод пользователя, шифрует его и отправляет на сервер, а ответ выводит в терминал. Опциональный четвертый аргумент отправляется как one-shot команда и завершает клиента после получения ответа.
- `common.c/.h` — общие типы пакетов, шифрование и контрольная сумма.
- `l2shell_kmod.c` — экспериментальный in-kernel транспорт-триггер: слушает кастомный EtherType, ловит init-пакет клиента и через `call_usermodehelper` запускает указанную команду, не поднимая пользовательских устройств. На время работы пользовательского сервера отключает перехват кадров, а после завершения процесса автоматически включает его снова.
...
1. **Инициализация контекста**
   - Читает аргументы `<iface> <server-mac> [remote-shell] [one-shot]` и флаги (`--local-echo`, `--skip-init`, `--tty-loop`), валидирует MAC, заполняет `client_ctx` (сокет, `sockaddr_ll`, дедуп-кэш). При `--skip-init` init-кадр не отправляется.
...
3. Клиент: `./b <iface> <mac> [remote shell cmd] [one-shot command] [--skip-init]` (пример: `./b eth1 11:22:33:44:55:66 /bin/bash "echo 123"`). Если удаленная сторона уже активна, можно добавить `--skip-init`, чтобы не слать стартовую команду. Пока сервер не ответил, клиент шлет broadcast.
...
- _пусто_
### Завершенные задачи
- [x] Устранить CRC mismatch за счет синхронизации порядка «encrypt→checksum» между клиентом/сервером: расчет контрольной суммы теперь идет по уже зашифрованному CRC-ключом пейлоаду, а приемник повторяет обе стадии дешифрования перед верификацией (включая `l2shell_kmod.c`). Параллельно убран дублирующий дамп кадров на сервере и обновлены unit-тесты.
...
- [x] Синхронизировать дампы кадров клиента и сервера, чтобы вывод отладки с обеих сторон отображал одинаковые байты и не путал зашифрованный/расшифрованный вид.
