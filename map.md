...
1. [ ] Перевести `client.c` и `server.c` на новый протокол, оставить только чтение/запись сокета и реакцию на типы сообщений, убрать дублирующий разбор/сборку заголовков.
1. [ ] Добавить IO-хелперы `l2s_send_all`/`l2s_recv_exact` с обработкой `EINTR` и частичных записей, использовать их в клиента и сервере.
1. [ ] Централизовать коды ошибок парсера (`L2S_ERR_SHORT`, `L2S_ERR_FMT` и т.п.) и проверку границ буфера.
1. [ ] Переразложить файлы: `src/common.h`, `src/proto.[ch]`, `src/io.[ch]`, минимальный интерфейс для ядра.
1. [ ] Внедрить лог-слой `l2s_log` с уровнями и макросами для замены разбросанных printf.
1. [ ] Добавить модульные тесты для `proto`/`io`/CRC и небольшой интеграционный тест client↔server без ядра.
1. [ ] Переименовать CLI в `l2s-client`/`l2s-server`, добавить флаги вместо позиционных аргументов, оставить короткие симлинки.
