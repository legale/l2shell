...
- `common.c/.h` — общие типы пакетов, шифрование и контрольная сумма, работа с сокетами.
...
2. [x] Декомпозировать большие функции (`client.c`, `server.c`) на короткие шаги.
3. [x] Добавить тайм-аут на стороне сервера: при отсутствии трафика от клиента `SERVER_IDLE_TICKS` закрывать PTY и выходить из цикла.
4. [x] Унифицировать логирование и диагностику через общие хелперы с форматом `param=value`.
5. [x] Добавить runtime-assertions в основные функции и покрыть их тестами.
6. [x] Доработать `common.c`, `client.c`, `server.c`: добавить HELLO (версия `0x01`, TLV `0x01`/`0x02`/`0x03`), чтобы клиент всегда отправлял обе записи, а сервер корректно парсил и запускал shell.
7. [x] Обновить модуль ядра: добавить поддержку HELLO версии `0x01` с теми же TLV и запускать сервер до возврата трафика.
8. [ ] Прогнать проверки (`make`, нужные тесты) и зафиксировать поведение HELLO/nonce в README или логах изменений. *(юнит-тесты и сборка kmod уже пройдены, осталось дописать документацию)*
