...
1. [ ] Подготовить OpenWrt-пакет: описать сборку/установку `a` и `b` в `l2shell/Makefile`, обеспечить установку в `/usr/bin/a` и `/usb/bin/b`.
1. [ ] Проверить `make test`, найти и исправить ошибки в клиенте/сервере, затем повторно прогнать тесты.
